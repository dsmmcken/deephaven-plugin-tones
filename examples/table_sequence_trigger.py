...
This is the table analogue of the ``play_sequence`` control method:
``chord_column`` plays simultaneous chords; ``sequence_column`` plays notes in
time. Both fire on a truthy trigger column and can even be combined on one
``table_tones(...)`` element (different trigger columns).

* ``Sparkle = Math.random() < 1.0/12.0`` -- ~1/12 of rows are flagged.
* ``table_tones(sequence_column="Sparkle", mode="all")`` -- each flagged row plays the
  ``sequence_notes`` arpeggio; other rows are silent.
...
your own to play any motif (same note forms as ``play_sequence``).
...
def sequence_trigger_panel():
    """Layout: invisible table_tones() engine + the live table with green motif rows."""
    return ui.flex(
        table_tones(
            table=_ticking,
            ...
            sequence_gap="16n",  # snappy arpeggio
