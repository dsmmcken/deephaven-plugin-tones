...
const jsRequire = createRequire(
  path.resolve(__dirname, "../../src/js/package.json"),
);
...
const handler = plugin.eventMapping[TONES_EVENT];
if (typeof handler !== "function") {
  fail(
    `eventMapping['${TONES_EVENT}'] is not a function (got ${typeof handler})`,
  );
}
ok(`eventMapping['${TONES_EVENT}'] is a handler`);

// ── Confirm tone was bundled inline (not external / not a sibling chunk) ─────
if (!/15\.1\.22|triggerAttackRelease|PolySynth/.test(text)) {
  fail(
    "tone does not appear to be bundled into index.js (no Tone.js symbols found)",
  );
}
ok(
  "tone is bundled inline into index.js (no sibling chunk required at runtime)",
);
